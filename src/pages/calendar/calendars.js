...
class CalendarList extends Component {
    ...
    componentWillReceiveProps(nextProps) {
        if (nextProps.data != null) {
            if(nextProps.requestType == 'calendar_get_dashboard_data'){
                if(nextProps.data.status == "success"){
                    this.setState({calendars: nextProps.data.calendar_data, data:this.getUpdatedDataSource(nextProps.data.calendar_data)});
                    this.arrayholder = nextProps.data.calendar_data;
                }
            }
            if(nextProps.requestType == 'delete_calendar'){
                let rows = [];
                let newArrayholder = [];
                for (let i = 0; i < this.state.dataSource.length; i ++) {
                    if (this.state.dataSource[i].id != this.state.delete_calendar_id) {
                        rows.push(this.state.dataSource[i]);
                    }
                }
                for(let i =0 ; i <this.arrayholder.length; i++){
                    if(this.arrayholder[i].id != this.state.delete_calendar_id){
                        newArrayholder.push(this.arrayholder[i]);
                    }
                }
                this.arrayholder = newArrayholder;
                this.setState({dataSource: rows, delete_calendar_id: ''});
    ...
    deleteCalendar(calendar_id){
        setTimeout(() => {
            Alert.alert(
                I18n.t('calendars.delete_calendar'),
                I18n.t('calendars.delete_message'),
                [
                    {
                        text: I18n.t('common.ok'), onPress: () => {
                            this.setState({delete_calendar_id: calendar_id});
                            this.props.deleteCalendar(calendar_id);
                        }
                    },
                    {
    ...
    searchFilterFunction(text) {
        ...
        const newData = this.arrayholder.filter(function(item) {
            ...
            return ( itemData.indexOf(textData) > -1 || itemDataDescription.indexOf(textData) > -1);
        });
    ...
    getUpdatedDataSource(newData){
        let rows = newData;
//        let rows = this.state.dataSource.concat(newData);
    ...
    onOpen(name, description, hash_code) {
        shareOptions = {
            title: name,
            message: description,
            url: Config.SITE_URL + "calendar/"+hash_code+"/detail",
            subject: "Share Calendar" //  for email
        };
                    ...
                    <View style={{flex: 1}}>
                        {(() => {
                            if (this.state.dataSource.length == 0 && !this.props.isLoading) {
                                return (
                                        <Text style={{textAlign: 'center', marginTop: 50, width: '100%'}}>{I18n.t('calendars.there_are_no_calendars')}</Text>
                                );
                            } else {
                                return (
                                        <View style={{marginLeft: 20, flex: 1}}>
                                            <ListView
                                                    ...
                                                    renderRow={data => {
                                                        return (
                                                                <ListItem style={{marginLeft:0, marginRight:0,  borderBottomWidth: 0, borderBottomColor: '#fff', marginBottom: 20}}>
                                                                    <Body style={{borderWidth: 1, borderColor: '#e1e6ef'}}>
                                                                    {(() => {
                                                                        if(data.picture !== '' && data.picture !== null && data.picture !== 'null' ){
                                                                            return (
                                                                                    <Image  style={{width:'100%', height: 100}} source={{uri: Config.SITE_URL +'uploads/' + data.picture}}/>
                                                                            );
                                                                        }
                                                                    })()}
                                                                    <View style={{padding: 10, borderBottomWidth: 1, borderBottomColor: '#e1e6ef', flex:1}}>
                                                                        <Text style={{fontSize: 18, marginBottom: 10}}>{data.name}</Text>
                                                                        <Text style={{fontSize: 14, marginBottom: 10}}>{data.description}</Text>
                                                                        ...
                                                                        <Grid>
                                                                            {(() => {
                                                                                if(this.props.role == "company" || this.props.role == "admin"){
                                                                                    return (
                                                                                            <Col onPress={()=>this.props.navigation.navigate("Events", {calendar_id : data.id})}>
                                                                                                <Icon name="md-eye" style={{width: '100%', fontSize: 20, textAlign: 'center'}}/>
                                                                                                <Text style={{fontSize:14, width: '100%', textAlign: 'center', marginLeft:0, marginRight:0 }}>{I18n.t('calendars.events_view')}</Text>
                                                                                            </Col>
                                                                                    );
                                                                                }
                                                                            })()}
                                                                            {(() => {
                                                                                if(data.public == "false"){
                                                                                    return (
                                                                                            <Col onPress = {() => this.onOpen(data.name, data.description, data.hash_code)}>
                                                                                                <Icon name="md-share" style={{width: '100%', fontSize: 20, textAlign: 'center'}}/>
                                                                                                <Text style={{fontSize:14, width: '100%', textAlign: 'center', marginLeft:0, marginRight:0 }}>{I18n.t('calendars.share')}</Text>
                                                                                            </Col>
                                                                                    );
                                                                                }
                                                                    ...
                                                                    <View style={{padding: 10, flex:1,  }}>
                                                                        <Grid>
                                                                            {(() => {
                                                                                if(this.props.role == "company" || this.props.role == "admin"){
                                                                                    return (
                                                                                            <Col onPress={()=>this.props.navigation.navigate("EditCalendar", {calendar_id : data.id})}>
                                                                                                <Icon name="md-create" style={{width: '100%', fontSize: 20, textAlign: 'center'}}/>
                                                                                                <Text style={{fontSize:14, width: '100%', textAlign: 'center', marginLeft:0, marginRight:0 }}>{I18n.t('common.edit')}</Text>
                                                                                            </Col>
                                                                                    );
                                                                                }
                                                                            })()}
                                                                            {(() => {
                                                                                if(this.props.role == "company" || this.props.role == "admin"){
                                                                                    return (
                                                                                            <Col onPress={() => this.deleteCalendar(data.id)}>
                                                                                                <Icon name="md-trash" style={{width: '100%', fontSize: 20, textAlign: 'center'}}/>
                                                                                                <Text style={{fontSize:14, width: '100%', textAlign: 'center',  marginLeft:0, marginRight:0 }}>{I18n.t('common.delete')}</Text>
                                                                                            </Col>
                                                                                    );
                                                                                }
                                                                            })()}
...
CalendarList.navigationOptions = ({navigation }) => ({
    header: (
            <Header style={s.menuHeader}>
                <Left>
                    <ButtonNative transparent onPress={() => navigation.navigate("DrawerOpen")}>
                        <Icon name="md-menu" style={s.menuIcon}/>
                    </ButtonNative>
                </Left>
                <Body>
                <Title style={{width: 230, color: '#fff'}}>{I18n.t('calendars.calendars')}</Title>
                </Body>
                <Right>
                    {(() =>{
                        ...
                        if(!_.isEmpty(params) && params.role == 'company'){
                            return(
                                <ButtonNative transparent onPress={() => navigation.navigate("AddCalendar")}>
...
const mapDispatchToProps = (dispatch) => {
    return {
        getCalendars: (search) => {
            dispatch(postRequest(Config.COMPANY_GET_DASHBOARD_DATA_URL,{search: search}, 'calendar_get_dashboard_data'));
        },
        deleteCalendar:(calendar_id) =>{
            dispatch(postRequest(Config.DELETE_CALENDAR_URL, {calendar_id: calendar_id}, 'delete_calendar'))
        },
        logout: () => {
            dispatch(logout());
        }
